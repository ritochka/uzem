...
<br/>
        ...
        <table class="table table-bordered" style="font-size: 12px;">
            <thead>
                <tr class="info">
                    <th colspan="3" style="text-align:center;"> {{{ trans('default.References') }}} </th>
                </tr>
            </thead>
            <tbody>
                <tr class="warning">
                    <td></td>
                    <td><strong> {{{ trans('default.References') }}} </strong></td>
                    <td><strong> {{{ trans('default.Type') }}} </strong></td>
                </tr>
                @foreach($course->literatures as $key => $literature)
                <tr>
                    <td style="width:20px; text-align:center;" valign="top">  {{{ $key }}}  </td>
                    <td> {{{ $literature->kaynak }}} </td> 
                    <td style="width:100px;"> {{{ $literature->turu }}} </td> 
                </tr>
                @endforeach
            </tbody>
        </table>
